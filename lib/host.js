...
//#region lib/types/host/rate-limit.js
/** GitHub sends reset headers on permission errors too; they alone do not prove throttling. */
function githubRetryAt(status, headers, message, now) {
	if (status !== 403 && status !== 429) return void 0;
	const remaining = headers.get("x-ratelimit-remaining");
	const retry = headers.get("retry-after");
	const primary = remaining === "0";
	if (!primary && retry === null && status !== 429 && !/rate limit|abuse detection/i.test(message)) return;
	const seconds = retry === null || retry.trim() === "" ? NaN : Number(retry);
	const retryAt = Number.isFinite(seconds) && seconds >= 0 ? now + seconds * 1e3 : retry === null ? NaN : Date.parse(retry);
	const reset = Number(headers.get("x-ratelimit-reset") ?? NaN);
	const deadlines = [retryAt, primary && Number.isSafeInteger(reset) && reset > 0 ? reset * 1e3 : NaN].filter(Number.isFinite);
	return deadlines.length > 0 ? Math.max(now + 1e3, ...deadlines.map((value) => value + 1e3)) : now + 6e4;
}
//#endregion
		...
		*/
		async search(request) {
			...
			if (!response.ok) {
				...
				if (retryAt !== void 0) throw new RemoteError("plugin-market/rate-limited", `GitHub search rate limit reached; retry after ${new Date(retryAt).toISOString()}`, { retryAt });
