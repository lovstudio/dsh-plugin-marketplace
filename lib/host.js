	...
	return class PluginMarketGitHubGateway extends _classSuper {
		...
		*/
		async resolvePackage(request) {
			...
			const registry = await fetch(`https://registry.npmjs.org/${manifest.name.split("/").map(encodeURIComponent).join("/")}`, { headers: { accept: "application/vnd.npm.install-v1+json" } });
			return {
